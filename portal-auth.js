...
function getAdminFilters() {
  ...
  return {
    ...
    query: searchFilter ? searchFilter.value.trim().toLowerCase() : ''
  };
}
...
function filterSubmissions(submissions, filters) {
  return submissions.filter((submission) => {
    ...
    const queryText = [
      ...
      submission.targetProgram,
      submission.summary
    ]
      .join(' ')
      .toLowerCase();
    const searchMatch = !filters.query || queryText.includes(filters.query);
    return statusMatch && searchMatch;
  });
}

function generateLetterText(submission, student, session, message) {
  return [
    'TOLBERT INNOVATION HUB',
    'Official Admission Letter',
    '',
    `Letter ID: ${submission.admissionLetter?.letterId || 'N/A'}`,
    `Date Issued: ${new Date(submission.admissionLetter?.issuedAt || new Date().toISOString()).toLocaleString()}`,
    '',
    `Student Name: ${student.fullName}`,
    `Student Email: ${student.email}`,
    `Application Type: ${submission.applicationType}`,
    `Target Program: ${submission.targetProgram}`,
    '',
    'Admission Notice:',
    message,
    '',
    `Issued By: ${session.name}`,
    'Tolbert Innovation Hub Admissions Office'
  ].join('\n');
}

function downloadLetterFile(filename, textContent) {
...
function renderAdminSubmissions() {
  ...
  const filtered = filterSubmissions(allSubmissions, getAdminFilters());

  if (!filtered.length) {
    host.innerHTML = '<p class="section-intro">No submissions match the current filters.</p>';
    return;
  }
      ...
      <article class="card admin-submission-item" data-student-id="${escapeHTML(submission.studentId)}" data-submission-id="${escapeHTML(submission.id)}">
        ...
        <form class="admin-letter-form form-wrap">
          <label>Admission Letter Message
            <textarea name="letterMessage" rows="3" required placeholder="Enter admission letter note for the student.">${escapeHTML(submission.admissionLetter?.message || '')}</textarea>
          </label>
          <div class="admin-actions">
            <button type="submit" class="btn btn-primary">Issue Admission Letter</button>
            ${submission.admissionLetter ? '<button type="button" class="btn btn-outline btn-download-letter">Download Letter</button>' : ''}
          </div>
...
function loadAdminDashboard() {
  ...
  const statusFilter = document.getElementById('admin-status-filter');
  const searchFilter = document.getElementById('admin-search-filter');
  if (statusFilter) statusFilter.addEventListener('change', renderAdminSubmissions);
  if (searchFilter) searchFilter.addEventListener('input', renderAdminSubmissions);

  const host = document.getElementById('admin-submissions');
  if (host) {
    host.addEventListener('click', (event) => {
      const saveButton = event.target.closest('.btn-save-status');
      if (saveButton) {
        const container = saveButton.closest('.admin-submission-item');
        if (!container) return;
      ...
      const downloadButton = event.target.closest('.btn-download-letter');
      if (!downloadButton) return;

      const container = downloadButton.closest('.admin-submission-item');
      if (!container) return;
      ...
      const students = getStudents();
      const match = findSubmission(students, studentId, submissionId);
      if (!match || !match.submission.admissionLetter) return;

      const text = generateLetterText(match.submission, match.student, session, match.submission.admissionLetter.message);
      const safeName = match.student.fullName.replace(/\s+/g, '-').toLowerCase();
      downloadLetterFile(`admission-letter-${safeName}-${match.submission.admissionLetter.letterId}.txt`, text);
    });
    ...
    host.addEventListener('submit', (event) => {
      ...
      const existingLetterId = match.submission.admissionLetter?.letterId;
      const letterId = existingLetterId || `TIH-ADMIT-${new Date().getFullYear()}-${Math.floor(Math.random() * 9000 + 1000)}`;
